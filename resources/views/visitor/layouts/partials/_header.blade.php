                ...
                <nav class="navbar navbar-expand-lg navbar-light navigation">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                                ...
                                <div class="dropdown-menu">
                                    @foreach ($categories as $category)
                                        <a class="dropdown-item"
                                            href="{{ route('visitor.show.books.by.categories', $category->id) }}">{{ $category->name }}</a>
                                    @endforeach
                                </div>
                            </li>
                            <li class="nav-item dropdown dropdown-slide">
                                ...
                                <div class="dropdown-menu">
                                    @foreach ($authors as $author)
                                        <a class="dropdown-item"
                                            href="{{ route('visitor.show.books.by.authors', $author->id) }}">{{ $author->name }}</a>
                                    @endforeach
                                </div>
                            </li>
                            {{-- <li class="nav-item dropdown dropdown-slide"> --}}
                        ...
                        <ul class="navbar-nav ml-auto mt-10">
                            @auth
                                <li class="nav-item dropdown dropdown-slide">
                                    <a class="nav-link dropdown-toggle profile-thumb" href="" data-toggle="dropdown"
                                        aria-haspopup="true" aria-expanded="false">
                                        <img src="{{ asset('assets/images/user/user-thumb.jpg') }}" width="36" height="36"
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <button class="dropdown-item logout-button text-danger" form="logout"><i
                                                class="fa fa-sign-out"></i>&nbsp; Logout</button>
                                    </div>
                                </li>
                                <form class="hide" action="{{ route('logout') }}" id="logout" method="POST">
                                    @csrf</form>
                            @else
                                <li class="nav-item">
