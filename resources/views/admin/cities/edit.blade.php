      ...
      <form method="POST" action="{{ route('admin.cities.update', $city->id) }}">
        ...
        <div class="card-body">
          ...
          <div>
            ...
            <x-adminlte-input name="name" value="{{old('name', $city->name)}}" placeholder="Enter name" required/>
          </div>
...
{{--           Status --}}
          <div class="">
            ...
            <x-adminlte-select name="status" required>
              <option value="1" @if(old('status', $city->status)) selected @endif >{{ __('Active') }}</option>
                <option value="0" @if(!old('status', $city->status)) selected @endif >{{ __('Inactive') }}</option>
            </x-adminlte-select>
          </div>
        </div>
        <!-- /.card-body -->
